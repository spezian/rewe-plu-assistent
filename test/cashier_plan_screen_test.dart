...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(find.text('2 laut Plan jetzt da'), findsOneWidget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      expect(find.text('2 laut Plan jetzt da'), findsNothing);
  ...
  testWidgets('renders the mobile preview', (tester) async {
    ...
    await mount(tester, PlanController(canEdit: true), boundary: boundary);
    final render =
        boundary.currentContext!.findRenderObject()! as RenderRepaintBoundary;
    await tester.runAsync(() async {
      final image = await render.toImage(pixelRatio: 2);
      final data = await image.toByteData(format: ui.ImageByteFormat.png);
      await File(screenshot!).writeAsBytes(data!.buffer.asUint8List());
      image.dispose();
    });
